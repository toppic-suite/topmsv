...
function importData(db: any, data: string) {
    ...
    let id: number = parseInt(stmtMaxSeqID.get().maxID) + 1;
    Papa.parse(data, {
        header:true,
        complete: function(results) {
            ...
            results.data.forEach(row => {
                if (row["Spectrum ID"] != "") {
                    ...
                    let scan: string = row["Scan(s)"];
                    // console.log('Proteoform:', row[17]);
                    let prec_mass: string = row["Precursor mass"];
                    let protein_accession: string = row["Protein accession"];
                    let proteoform: string = row["Proteoform"];
                    let qValue: string = row["Spectrum-level Q-value"];//spectral q-value
                    let eValue: string = row["E-value"];
