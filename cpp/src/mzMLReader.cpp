...
/*void mzMLReader::setColor(std::vector<peakProperties> &all_ms1_peaks){
...
/*void mzMLReader::insertPeakDataToGridBlocks(int table_cnt){
  ...
  for (double i = Range.rt_min; i <= Range.rt_max; i += Range.rt_size){
    ...
    rc_ = sqlite3_exec(db_, sql_, callbackConvertData, grid_ptr, &z_err_msg_);//after this function, gridBlocks has a peak for each grid
