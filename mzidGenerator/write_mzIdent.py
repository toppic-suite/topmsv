...
class IdentiPyMzIdentMLWriter(object):
    def __init__(self, param, data, db_data, total_ptm, user_ptm):
        self.param = param
        self.data = data
        self.db_data = db_data
        self.total_ptm = total_ptm
        self.user_ptm = user_ptm
        self.spec_id = 0
    ...
    def transform_peptide(self, prsm):
        ...
        for i in range(0,  len(seq)):
            if seq[i] == "[":
                ...
                if massShift != '':
                    temp_pos = position;
                    if massShift[-1].isnumeric():#unknown shift
    ...
    @classmethod
    def write_mzid(cls, param, data, db_data, total_ptm, user_fixed_ptm):
        writer = cls(param, data, db_data, total_ptm, user_fixed_ptm)
...
if __name__ == "__main__":
    ...
    user_ptm = parse_user_ptm(user_localize_ptm_path)#empty if no file was used

    total_ptm = user_fixed_ptm + ptm_data + user_ptm#all ptm used for sequence annotation
    IdentiPyMzIdentMLWriter.write_mzid(param, data, db_data, total_ptm, user_fixed_ptm)
