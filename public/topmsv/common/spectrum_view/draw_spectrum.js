 ...
 */
function onPeakMouseOut(this_element) {
    onMouseOut();
...
function onFragmentMassAndIonTypeMouseOut(this_element) {
    onMouseOut();
    d3.select(this_element).style("stroke", "black")
 ...
 */
function onMouseOverPeak(this_element, event, peak, para) {
    let intensity = " inte:" + peak.getIntensity().toFixed(3);
    let pos = peak.getPos().toFixed(3);
 ...
 * @param {object} para - Contains the parameters like height, width etc.,. tht helps to draw the graph
 */
function onMouseOverCircle(this_element, event, envelope, peak) {
    let mz = "m/z:" + peak.getPos().toFixed(3);
 ...
 */
function onMouseOverFragmentMassAndIonType(this_element, event, mass, ionData) {
    let pos = mass.toFixed(3);
...
function drawTicks(svg, para) {
    ...
    for (let i = 0; i <= para.getYTickNum(); i++) {
        ...
        let tickPos = i * tickHeight;
        //* para.dataMaxInte /100;
        let y = para.getPeakYPos(tickHeight);
...
function addDatatoAxis(svg, para) {
    let maxMz = para.getWinMaxMz();
    let minMz = para.getWinMinMz();
    ...
    for (let i = 0; i < xTickPosList.length; i++) {
        ...
        if (x >= para.getPadding().left &&
            ...
            xAxisData.append("text").attr("id", "xtext").attr("x", x)
                ...
                if (maxMz - minMz <= 0.0001)
                    return tickMz.toFixed(6);
                else if (maxMz - minMz <= 0.001)
                    return tickMz.toFixed(5);
                else if (maxMz - minMz <= 0.01)
                    return tickMz.toFixed(4);
                else if (maxMz - minMz <= 0.1)
                    return tickMz.toFixed(3);
                else if (maxMz - minMz <= 1)
                    return tickMz.toFixed(2);
                else if (maxMz - minMz <= 3)
                    return tickMz.toFixed(2);
                else if (maxMz - minMz <= 5)
                    return tickMz.toFixed(1);
                return tickMz;
            })
...
function addHighlight(svg, para) {
    ...
    if (!((para.getHlMinMz() < para.getWinMinMz()
        ...
        let x = para.getPeakXPos(para.getHlMinMz());
        let x2 = para.getPeakXPos(para.getHlMaxMz());
        if (para.getHlMinMz() < para.getWinMinMz()) {
...
function drawEnvelopes(svg, para, envList) {
    ...
    let maxIntensity = para.getDataMaxInte();
    let spectrumData = new SpectrumFunction();
    // limits provide current count of number of peaks drawn on graph per bin(range)
 ...
 * @param {object} para - Contains the parameters like height, width etc.,. tht helps to draw the graph
 */
function addErrorPlot(svg, para) {
    //Draw x-axis
