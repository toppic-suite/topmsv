...
type matchedUnMatchedPeak = {
	"peak": MatchedPeakEnvelopePair,
	"isMatchedPeak": boolean
}
...
class CalcMatchedPeaks {
	 ...
	 */
	matchedPeakAttributes(monoMassList_temp: Peak, id: string, ion: string, ionPos: number,
		position: number, massDiff: number, mass: number, matchedInd: string)
	{
		...
		if(matchedInd == this.CONST_Y)
		{
			...
			let ionTerm: string | null = getIonTerminal(ion);
			
			if (ionTerm) {
				let ionObj = new Ion(ionPos.toString(), ion, ionTerm, 0, massDiff, PPMerror); 
				let matchedPeak: MatchedPeakEnvelopePair = new MatchedPeakEnvelopePair(thMass, peak, ionObj);
			}
	 ...
	 */
	getMatchedPeakList(prefixOrSuffixMassList: TheoMass[], monoMassList: Peak[],sequence,massErrorthVal,ppmErrorthVal,ionObj, matchedPairList)
	{
		let matchedList = [];
