...
onMouseOverPeak = function(this_element,peak,para) {
  ...
  let pos = parseFloat(peak.getPos()).toFixed(3);
...
function drawPeaks(svg,para,peakList){
  ...
  {
    let peak = peakList[i];
    if(peak.getPos() >= para.winMinMz && peak.getPos() < para.winMaxMz)
    {
      if (peak.getDisplayLevel() / (spectrumData.mzLevel.length - 3)>= ratio || ratio <= 0.2){
        peaks.append("line")
        .attr("x1",function(){
...
function drawEnvelopes(svg,para,envList) {
  ...
  envList.forEach(env => {
    ...
    if(peaks[0].getPos() >= para.winMinMz && peaks[0].getPos() < para.winMaxMz) 
    { 
      if (env.getDisplayLevel() / (spectrumData.mzLevel.length - 3) >= ratio || ratio <= 0.2){
