...
"use strict";
function geneDataObj(callback) {
    let protObj = geneProteoform();
    genePrsm(protObj, function (prsmObj) {
        callback(prsmObj);
    });
}
function genePrsm(proteoformObj, callback) {
    ...
    let [nIons, cIon] = parseIons();
    geneMs1Spectrum(nIons, cIon, (ms1Spec) => {
        geneMs2Spectrum(nIons, cIon, (ms2Spec) => {
            let machedPeakEnvPair = [];
            ms2Spec.forEach((spectra) => {
                machedPeakEnvPair = machedPeakEnvPair.concat(geneMatchedPeakEnvelopePairs(spectra.getSpectrumId(), spectra.getEnvs()));
            });
...
function geneMs1Spectrum(nIons, cIons, callback) {
    let prsm = prsm_data.prsm;
    let ms1SpecId = prsm.ms.ms_header.ms1_ids.split(" ")[0];
    let filename = "../../topfd/ms1_json/spectrum" + ms1SpecId + ".js";
...
function geneMs2Spectrum(nIons, cIons, callback) {
    let prsm = prsm_data.prsm;
    ...
    for (let i = 0; i < specIdList.length; i++) {
        let ms2Filename = "../../topfd/ms2_json/spectrum" + specIdList[i] + ".js";
...
function geneMatchedPeakEnvelopePairs(specId, envelopes) {
    let matchedPairList = [];
