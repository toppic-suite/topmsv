...
export function showEnvTable(scan: string): void {
  ...
  let resultViz: ResultViz = new ResultViz(projectDir.value);
  let format: {"floatDigit": number, "scientificDigit": number} = resultViz.getConfig();
  ...
  $.ajax( {
    ...
    success: function (res) {
      //console.log(res);
      if(res!== '0') {
        let protData: {"seq": string, "prec_mass": string, "q_value": string, "e_value": string} = JSON.parse(res);
        let sequence: string = preprocessSeq(res);
