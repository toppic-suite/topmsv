(function () {
  ...
  // 进度条
  function setProgress(event: ProgressEvent) {
...
$( document ).ready(function() {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                for (let i: number = 0; i < res.length; i++) {
                    let eid: string = res[i].eid;
                    let ename: string = res[i].ename;
    ...
    $('#createButton').click(function () {
        var result: boolean = confirm("Are you sure that you want to create this dataset?");
        if (result) {
