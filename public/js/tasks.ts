function showTaskStatus(taskData: string, projectCode: string): void {
  let fileName: string = projectCode + "_" + taskData + "_log.txt";
  ...
  xhttp.onload = () => {
    if (xhttp.status == 200 && xhttp.readyState == 4) {
      ...
      let taskStatusPara = document.querySelector<HTMLParagraphElement>("#task-status-para");
      if (taskStatusPara) {
